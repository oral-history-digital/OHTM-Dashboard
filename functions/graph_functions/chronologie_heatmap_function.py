    ...
    if interview_id is not None:
        ...
        for sents in ohtm_file["corpus"][archive_id][interview_id]["sent"]:
            if (
                ...
                transfer.append(
                    [
                        ohtm_file["corpus"][archive_id][interview_id]["sent"][sents][
                            "tape"
                        ],
                        ohtm_file["corpus"][archive_id][interview_id]["sent"][sents][
                            "time"
                ...
                for chunk in dff:
                    ...
                    for segment in chunk:
                        ...
                        if tape_counter == int(segment[0]):
                            x = frames_to_timecode(
                                timecode_to_frames(segment[1], 24) + time_set, 24
                            )
                            transfer_segments.append(
                                [segment[0], x, segment[2], segment[3], segment[4]]
                            )
                        if tape_counter != int(segment[0]):
                            tape_counter += 1
        ...
        for i, count in enumerate(results_speaker_diffusion):
            ...
            if tc_indicator:
                for j, speaker in enumerate(count):
                    try:
                        final_results_speaker_diffusion.append(
                            [
                                speakers[j],
                                timecode_to_frames(dff_2[i][-1][1], 24) / 24 / 60,
                                speaker / wordcount[i],
                                i,
                            ]
                        )
                ...
                for j, speaker in enumerate(count):
                    try:
                        final_results_speaker_diffusion.append(
                            [speakers[j], i, speaker / wordcount[i]]
                        )
                    except ZeroDivisionError:
        ...
        if tc_indicator:
            for i, chunk in enumerate(dff_2):
                time = (
                    timecode_to_frames(chunk[-1][1], 24)
                    - timecode_to_frames(chunk[0][1], 24)
                ) / 24
                try:
                    wordfreq = wordcount[i] / time
                    results_worddensity.append(
                        [
                            "worddensity",
                            timecode_to_frames(chunk[-1][1], 24) / 24 / 60,
                            wordfreq,
                            i,
                        ]
                    )
                ...
                for nr, chunk in enumerate(dff_2):
                    for nr_c, cluster in enumerate(clusters):
                        ...
                        for top in cluster[0][1]:
                            transfer += ohtm_file["weight"][archive_id][interview_id][
                                str(chunk[0][4])
                            ][str(top)]
                        topicweights.append(
                            [
        ...
        # filter for a reduced and more consistent heatmap - topics below a certain threshold will be filtered out
            ...
            for c in ohtm_file["weight"][archive_id][interview_id]:
                ...
                for t in ohtm_file["weight"][archive_id][interview_id][c]:
                    if t not in heat_dic:
                        heat_dic.update(
                            {t: ohtm_file["weight"][archive_id][interview_id][c][t]}
                        ...
                        heat_dic.update(
                            {
                                t: heat_dic[t]
                                + ohtm_file["weight"][archive_id][interview_id][c][t]
                            }
                        )
                        if (
                            ohtm_file["weight"][archive_id][interview_id][c][t]
            ...
            if tc_indicator:
                for nr, chunk in enumerate(dff_2):
                    for top in ohtm_file["weight"][archive_id][interview_id][
                        str(chunk[0][4])
                    ]:
                        if top not in top_filter:
                            ...
                            topicweights.append(
                                [
                                    topic_labels[top],
                                    timecode_to_frames(chunk[-1][1], 24) / 24 / 60,
                                    ohtm_file["weight"][archive_id][interview_id][
                                        str(chunk[0][4])
                                    ][top],
                                    nr,
                                ]
        ...
        if tc_indicator:
            chronology_df = pd.DataFrame(
                topicweights, columns=["top", "minute", "weight", "ind"]
            )
        if tc_indicator == False:
            chronology_df = pd.DataFrame(
                topicweights, columns=["top", "chunk", "weight"]
            )
        ...
        if tc_indicator:
            df_heatmap = pd.DataFrame(
                topicweights, columns=["top", "minute", "weight", "ind"]
            )  # kann man bei Timecode auf X-Achse Chunk-Nr. ins Hover schreiben? - ist in der Tabelle schon als "ind"
            doc_tops_heatmap = df_heatmap.pivot(
                index="top", columns="minute", values="weight"
            )
            ...
            doc_tops_heatmap = df_heatmap.pivot(
                index="top", columns="chunk", values="weight"
            )
        ...
        fig_final.add_trace(
            go.Heatmap(
                z=doc_tops_heatmap, x=doc_tops_heatmap.columns, y=doc_tops_heatmap.index
            ),
            row=1,
            col=1,
        )
        ...
        fig_final.update_yaxes(
            visible=True, showticklabels=True, type="category", row=1, col=1
        )  # type = "category": equal y-axis ticks bei uneven number steps

        # speaker chronology
        if tc_indicator:
            df_heatmap = pd.DataFrame(
                final_results_speaker_diffusion,
                columns=["top", "minute", "weight", "ind"],
            )
            doc_tops_heatmap = df_heatmap.pivot(
                index="top", columns="minute", values="weight"
            )
            ...
            df_heatmap = pd.DataFrame(
                final_results_speaker_diffusion, columns=["top", "chunk", "weight"]
            )
            doc_tops_heatmap = df_heatmap.pivot(
                index="top", columns="chunk", values="weight"
            )
        ...
        if tc_indicator:
            df_heatmap = pd.DataFrame(
                results_worddensity, columns=["top", "minute", "weight", "ind"]
            )
            doc_tops_heatmap = df_heatmap.pivot(
                index="top", columns="minute", values="weight"
            )
            fig = px.imshow(
                doc_tops_heatmap, color_continuous_scale="dense", aspect="auto"
            )
            ...
            fig_final.add_trace(
                go.Heatmap(
                    z=doc_tops_heatmap,
                    x=doc_tops_heatmap.columns,
                    y=doc_tops_heatmap.index,
                ),
                row=3,
                col=1,
            )
        ...
        fig_final.update_traces(
            hovertemplate="Chunk: %{x}<br>Topic: %{y}<br>Weight: %{z}<extra></extra>",
            row=1,
            col=1,
        )
        if tc_indicator:
            ...
            if "marker" in heatmap_filter:
                if tc_indicator:
                    row_index_clicked = chronology_df.index.get_loc(
                        chronology_df[
                            chronology_df["minute"] == click_data_2["points"][0]["x"]
                        ].index[0]
                    )
                    ...
                    if chunk_number_clicked == 0:
                        ...
                        chunk_number_clicked
                        == chronology_df["ind"][chronology_df.index[-1]]
                    ):
                        row_index = chronology_df.index.get_loc(
                            chronology_df[
                                chronology_df["ind"] == chunk_number_clicked
                ...
                if (
                    ctx_triggered[0]["prop_id"] == "chunk_number_frontpage.data"
                    or "chunk_number_detail.data"
                ):
                    if tc_indicator:
                        if chunk_number_storage == 0:
                            row_index = chronology_df.index.get_loc(
                                chronology_df[
                                    chronology_df["ind"] == chunk_number_storage
                                ].index[0]
                            )
                            time_id = chronology_df.loc[row_index]["minute"]
                            row_index_after = chronology_df.index.get_loc(
                                chronology_df[
                                    chronology_df["ind"] == chunk_number_storage + 1
                                ].index[0]
                            )
                            time_id_after = chronology_df.loc[row_index_after]["minute"]
                            x_1 = (time_id + time_id_after) / 2
                            ...
                            time_id_before = chronology_df.loc[row_index_before][
                                "minute"
                            ]
