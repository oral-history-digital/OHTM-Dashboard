...
def heatmap_marker_creation_chronic(
    chronology_df_detail,
    heatmap_filter,
    tc_indicator_detail,
    clickData_2,
    ...
    if "mark" in heatmap_filter:
        if tc_indicator_detail:
            row_index_clicked = chronology_df_detail.index.get_loc(
                chronology_df_detail[
                    chronology_df_detail["minute"] == clickData_2["points"][0]["x"]
                ].index[0]
            )
            ...
            if chunk_number_clicked == 0:
                ...
                == chronology_df_detail["ind"][chronology_df_detail.index[-1]]
            ):
                row_index = chronology_df_detail.index.get_loc(
                    chronology_df_detail[
                        chronology_df_detail["ind"] == chunk_number_clicked
        ...
        if ctx.triggered[0]["prop_id"] == "chunk_number_detail.data":
            if tc_indicator_detail:
                if chunk_number_storage == 0:
                    row_index = chronology_df_detail.index.get_loc(
                        chronology_df_detail[
                            chronology_df_detail["ind"] == chunk_number_storage
                        ].index[0]
                    )
                    time_id = chronology_df_detail.loc[row_index]["minute"]
                    row_index_after = chronology_df_detail.index.get_loc(
                        chronology_df_detail[
                            chronology_df_detail["ind"] == chunk_number_storage + 1
                        ].index[0]
                    )
                    time_id_after = chronology_df_detail.loc[row_index_after]["minute"]
                    ...
                    time_id_before = chronology_df_detail.loc[row_index_before][
                        "minute"
                    ]
